...
| 页面层 | 主入口界面、菜单栏入口、浮窗 SwiftUI 内容 | `CinemaMode/Views/` |
...
| `MainControlView` | 提供进入观影模式的普通入口 | Services | AppKit 细节 |
...
| 控制项 | MVP 策略 | 备注 |
|--------|----------|------|
| 菜单栏 | 使用 AppKit presentation options 隐藏或自动隐藏 | 进入前保存原始 options |
