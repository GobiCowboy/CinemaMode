...
| 层级 | 职责 | 示例目录 |
|------|------|----------|
| App 层 | 菜单栏入口、scene 定义、生命周期恢复 | `Sources/CinemaMode/App/` |
| 页面层 | 菜单栏入口、浮窗 SwiftUI 内容 | `Sources/CinemaMode/Views/` |
...
| 模块 | 职责 | 允许依赖 | 禁止依赖 |
|------|------|----------|----------|
| `CinemaModeApp` | 声明菜单栏入口、注入服务、处理启动恢复 | Services, Support | 直接操作 `NSApplication.presentationOptions` |
