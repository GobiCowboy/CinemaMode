...
| 2 | `Sources/CinemaMode/Services/SystemPresentationController.swift` | 实现 `restore(from:)` | 恢复进入前 options |
| 3 | `Sources/CinemaMode/Services/SystemPointerActivityMonitor.swift` | 退出时停止监听 | 退出后不再触发透明度更新 |
| 4 | `Sources/CinemaMode/Services/FloatingPanelController.swift` | 退出时关闭浮窗 | 浮窗窗口释放 |
| 5 | `Sources/CinemaMode/App/CinemaModeApp.swift` | 启动时建立状态栏入口与服务 | 异常退出后不残留沉浸状态 |
| 6 | `Sources/CinemaMode/Services/SystemLogger.swift` | 记录退出和恢复日志 | 成功、失败、重试可排查 |
...
| `SystemPresentationController.restore(from:)` | function | `PresentationSnapshot` | Void | 恢复系统显示状态 |
| `FloatingPanelController.hide()` | function | 无 | Void | 关闭退出浮窗 |
...
| 已有类似实现？ | 需要复用 301 的 `SystemPresentationController` |
...
| `Sources/CinemaMode/App/CinemaModeApp.swift` | 生命周期兜底恢复 | 已创建 |
