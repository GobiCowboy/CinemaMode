...
| 页面 / 区域 | 元素 | 交互行为 | 状态 |
|-------------|------|----------|------|
| 右上角浮窗 | 圆形按钮 | 显示 🎬，无边框，可拖动 | active |
| 右上角浮窗 | opacity | 鼠标静止约 5% | idle pointer |
| 右上角浮窗 | opacity | 鼠标移动约 70% | moving pointer |
| 右上角浮窗 | opacity | 鼠标悬停 100% | hovering pointer |
| 右上角浮窗 | 点击 | 调用 `CinemaModeService.exit()` | active |
| 右上角浮窗 | 拖动 | 改变浮窗位置，不做吸附或边界约束 | active |

## 5. 涉及数据

| 数据 | 来源 | 用途 | 对应模型 |
|------|------|------|----------|
| 浮窗锚点 | 默认值 / 运行时位置 | 定位初始右上角 | `FloatingWindowState` |
...
| 验收项 | 判断方式 | 结果 |
|--------|----------|:----:|
| 默认右上角 | `swift test` + `./script/build_and_run.sh --verify` | 已通过 |
| 鼠标静止 5% | 单测 + 代码审查 | 已通过 |
| 鼠标移动 70% | 单测 + 代码审查 | 已通过 |
| 鼠标悬停 100% | 代码审查 | 已通过 |
| 可自由拖动到其他位置 | 手动验收 | 待验收 |
